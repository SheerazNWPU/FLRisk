...
def my_truncated_normal_ppf(confidence, a, b, mean, stddev, label_num, machine_mus, num_simulations=10000):
    ...
    #    num_simulations = num_simulations[0].item()   # Convert scalar tensor to int
...
class RiskModel(nn.Module):
    ...
    def forward(self, machine_labels, rule_mus, machine_mus, rule_feature_matrix, machine_feature_matrix,
        ...
        #print('machine mus')
        #print(machine_mus_vector.shape)
        #print('rule ms')
        #print(rule_mus.shape)
        #print('rule w')
        #print(self.rule_w.shape)
        ...
        #Fr_alpha = my_truncated_normal_ppf(self.alaph, self.a, self.b, big_mu, torch.sqrt(big_sigma), self.label_num, machine_mus_vector, 'mixture')
        #Fr_alpha_bar = my_truncated_normal_ppf(1 - self.alaph, self.a, self.b, big_mu, torch.sqrt(big_sigma), self.label_num, machine_mus_vector, 'mixture')
        ...
        #print('prob_mul')
        #print(prob_mul.shape)
        #prob_mul = prob_mul.mean(dim=1)
        #print(prob_mul.shape)
        prob_mul1 = convert_tensor(prob_mul)
        #prob_mul1 = torch.mean(prob_mul1, dim=2) 
        #print('prob mul 1')
        #rint(prob_mul1.shape)
        #print(prob_mul1)
...
def predict(model, test, epoch, init_rule_mu, init_machine_mu, epoch_cnn=0, is_print=False):
    ...
    with torch.no_grad():
        for i in range(batch_num):
            ...
            # Ensure big_mu_pred has correct label dimension
            if big_mu_pred.shape[1] != label_num:
                print(f"Warning: big_mu_pred shape mismatch. Expected {label_num} labels, got {big_mu_pred.shape[1]}")
                # Either pad or truncate to match
                if big_mu_pred.shape[1] < label_num:
                    # Pad with zeros
                    big_mu_pred = torch.nn.functional.pad(big_mu_pred, (0, label_num - big_mu_pred.shape[1]))
                else:
                    # Truncate
                    big_mu_pred = big_mu_pred[:, :label_num]
            ...
            # Apply the same fix to other predictions
            fr_pred = fr[0][:len(true_labels_batch)].clone().detach().to(dtype=torch.float, device=device[0])
            if fr_pred.shape[1] != label_num:
                if fr_pred.shape[1] < label_num:
                    fr_pred = torch.nn.functional.pad(fr_pred, (0, label_num - fr_pred.shape[1]))
                else:
                    fr_pred = fr_pred[:, :label_num]
            ...
            if fr_bar_pred.shape[1] != label_num:
                if fr_bar_pred.shape[1] < label_num:
                    fr_bar_pred = torch.nn.functional.pad(fr_bar_pred, (0, label_num - fr_bar_pred.shape[1]))
                else:
                    fr_bar_pred = fr_bar_pred[:, :label_num]
            ...
            if rs_pred.shape[1] != label_num:
                if rs_pred.shape[1] < label_num:
                    rs_pred = torch.nn.functional.pad(rs_pred, (0, label_num - rs_pred.shape[1]))
                else:
                    rs_pred = rs_pred[:, :label_num]
            ...
            if ms_pred.shape[1] != label_num:
                if ms_pred.shape[1] < label_num:
                    ms_pred = torch.nn.functional.pad(ms_pred, (0, label_num - ms_pred.shape[1]))
                else:
                    ms_pred = ms_pred[:, :label_num]
    ...
    #if is_print:
    #    print('epoch %d' % epoch)
    #    print('risk mul roc_auc:', risk_mul_roc_auc)
    #    print('baseline mul roc_auc:', baseline_mul_roc_auc)
    #    print('risk roc_auc:', risk_roc_auc)
    #    print('baseline roc_auc:', baseline_roc_auc)
    
    # Return the outputs_all tensor
    return outputs_all
