...
def get_one_knn_count(
    ...
    for k in k_list:
        ...
        for data_set in data_sets:
            ...
            #for i in range(len(paths)):
            #    for j in range(len(labels[i])):  # Assuming each labels[i] has a length of 3 as given
            #        # Compute class number based on label value (0 or 1) and its position in the list
            #        class_number = (2 * j + int(labels[i][j])) % len(count[i])
            
                    # Append the data for this particular path and class
            #        temp_csv.append([
            #            "{}_{:0>3d}".format(paths[i], class_number),
            #            1 if labels[i][j] == 1 else 0,  # Directly use the label to set class membership
            #            count[i][class_number],
            #        ])
