...
def load_data(cfg, client_name):
    ...
    # -- Train data --
    train_info = pd.read_csv(os.path.join(cfg.get_parent_path(client_name), 'train.csv')).values
    ...
    # -- Validation data --
    valida_info = pd.read_csv(os.path.join(cfg.get_parent_path(client_name), 'val.csv')).values
    ...
    # -- Test data --
    test_info = pd.read_csv(os.path.join(cfg.get_parent_path(client_name), 'test.csv')).values
    ...
    while i < len(df.columns):
        ...
        i += 1
    rules = rp.read_rules(cfg.get_decision_tree_rules_path(client_name))
    print('-----------len rule')
    print(len(rules))
